...
for FLAG in "$@"; do
    case "${FLAG}" in
        -h)
            ...
            println "  --staged        run only the language sections the index actually touches ( the pre-commit hook )"
...
RUN_JS_SECTIONS="true"

if [[ "true" = "${STAGED_ONLY_REQUESTED}" ]]; then
    RUN_PHP_SECTIONS="$(has_staged_change '\.php$')"
    RUN_JS_SECTIONS="$(has_staged_change '\.tsx?$')"

    if [[ "false" = "${RUN_PHP_SECTIONS}" ]] && [[ "false" = "${RUN_JS_SECTIONS}" ]]; then
        exit 0
    fi
fi
...
if [[ "true" = "${RUN_JS_SECTIONS}" ]]; then
    run_section "typecheck" "${TAG_VALIDATE}" "js" -- \
        run_in_service_shell "${CONTAINER_DEV}" "cd assets/react && npm run --silent typecheck"

    run_section "test" "${TAG_VALIDATE}" "js" -- \
        run_in_service_shell "${CONTAINER_DEV}" "cd assets/react && node --test"
fi
